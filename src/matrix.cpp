...
Matrix::Matrix(int rows, int cols , float element = 0) 
{
    if(cols <= 0 || rows <= 0)
    {
        throw std::invalid_argument("received negative or 0 value for either the cols or rows");
    }
...
Matrix Matrix::hadamardProduct(const Matrix& m) const
{
    if(m.getRows() != this->getRows() || m.getCols() != this->getCols())
        throw std::invalid_argument("the matrices have to have the same row and col number");
    
    auto result = Matrix(this->getRows(),this->getCols());
